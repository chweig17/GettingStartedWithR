...
I'm not too sure about this question. Maybe the date variables should be numberic instead of characters? 
...
DF$Photo.Time <- hms::as_hms(DF$Photo.Time)
```
...
```{r}
DF1<- DF %>%
  select(Sampling.Unit.Name, ndays, bin)

DF2 <- select(DF, Sampling.Unit.Name, ndays, bin)
```

2. Determine how many unique species were detected during Round 1 of the study.

```{r}
unique(DF$bin, incomparables = FALSE)
DF %>%
  group_by(bin) %>%
  summarize(count=n())

DF3<- as.data.frame(table(DF1$bin))
nrow(DF3)
```
There are 31 unique species (minus 1 for nothing here level)
Did this a couple of different ways 
...
```{r}
sum(DF$ndays)
DF4<- as.data.frame(table(DF$ndays))
DF4$Var1 <- as.numeric(as.character(DF4$Var1))
sum(DF4$Var1)
